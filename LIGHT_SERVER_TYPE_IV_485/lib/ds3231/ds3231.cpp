...
char *DS3231::getTimestamp()
{

    int dayOfWeek, date, month, year, hours, minutes, seconds;
    readDateTime(&dayOfWeek, &date, &month, &year, &hours, &minutes, &seconds);
    sprintf(charbuf, "%04d-%02d-%02d %02d:%02d:%02d", year, month, date, hours, minutes, seconds);
    return charbuf;
}
