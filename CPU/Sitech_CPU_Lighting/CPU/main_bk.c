...
// Bu#ild: gcc -Wall -lev -luwsc -o Cpu main.c Parameter_CPU.c function_system.c Modbus_Master_MFM384.c Sitech_Json.c -lwiringPi -lpthread -lmodbus -ljson-c -lev -luwsc -I.
...
void send_data_cpu_to_qt(int arr[], int rear, const char* template, const char cmd)
{
    ...
    for(int i = 0 ; i <= rear; i ++){
        if(arr[i] != 0)
        {
            snprintf(tmp_message2, sizeof(tmp_message2), "%s %d", tmp_message1, arr[i]);
...
void *ModeBus(void *threadArgs)
{
    ...
    while (1)
    {
        ...
        if (is_flag_opened_websocket)
        {
            ...
            // sprintf(topicpub12, "%s%s%s", topicraw, mac_final, topicrawpub12);
            // char message3[1280];
            // snprintf(message3, sizeof(message3), "{\"pkw\": %.02f, \"pkwh\": %.02f, \"val1\": %d, \"val2\": %d, \"V1\": %.02f, \"V2\": %.02f, \"V3\": %.02f, \"I1\": %.02f, \"I2\": %.02f, \"I3\": %.02f, \"P1\": %.02f, \"P2\": %.02f, \"P3\": %.02f, \"Pf1\": %.02f, \"Pf2\": %.02f, \"Pf3\": %.02f}"
            // ,parameter_MFM384.P_Total_MFM384, parameter_MFM384.P_Consumtion_kWH,
            // counters_package.val1, counters_package.val2,
            // parameter_MFM384.V1_MFM384,
            // parameter_MFM384.V2_MFM384,
            // parameter_MFM384.V3_MFM384,
            // parameter_MFM384.I1_MFM384,
...
void *main_process(void *threadArgs)
{
    ...
    while (1)
    {
        ...
        if (is_flag_opened_websocket)
        {
            ...
            {
                // printf("reach thread main process\r\n");
                if (digitalRead(auto_Mode) == 0)
                {
                    ...
                    if (digitalRead(auto_Mode) == 0)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if ((c_check_time >= c_start_time && c_check_time <= c_end_time) || (c_check_time >= c_start_time1 && c_check_time <= c_end_time1))
                                {
                                    digitalWrite(relay_1, 0);
                                    digitalWrite(relay_2, 0);
                                    if (is_status_power == false)
                            ...
                            {
                                // printf("Case 2\r\n");
                                if ((((c_check_time >= c_start_time || c_check_time <= c_end_time)) || ((c_check_time >= c_start_time1) && (c_check_time <= c_end_time1))) && (c_start_time > c_start_time1))
                                {
                                    digitalWrite(relay_1, 0);
                                    digitalWrite(relay_2, 0);
                                    if (is_status_power == false)
                            ...
                            {
                                if ((((c_check_time >= c_start_time1) || (c_check_time <= c_end_time1)) || ((c_check_time >= c_start_time) && (c_check_time <= c_end_time))) && (c_start_time1 > c_start_time))
                                {
                                    digitalWrite(relay_1, 0);
                                    digitalWrite(relay_2, 0);
                                    if (is_status_power == false)
    ...
    else
    {
        if (!strncmp(data, "qtApp-", 6))
        {
            ...
            case CMD_QT_TO_CPU_CHANGE_ID_RF:
                ...
                printf("parsing change id rf:%d, %d\r\n", changeid.oldid, changeid.newid);
            ...
            case CMD_QT_TO_CPU_OPEN_SERVER:
                ...
                Write_struct_setting_toFile(setting_file, "/home/pi/setting.txt", setting);
                delay(2000);
                if (flag_checkreboot != setting.server)
                    // system("sudo reboot");
                    break;
            ...
            case CMD_CPU_TO_QT_OPEN_SERVER:
                ...
                printf("%s\r\n", data_server);
                break;
            // case CMD_CPU_TO_QT_TYPE:
            // printf("Set type tio QT!\r\n");
            // int data_len_type = sprintf(data, TEMPLATE_TYPE, CMD_CPU_TO_QT_TYPE, setting.type);
            // cl->send_ex(cl, UWSC_OP_TEXT, 1, data_len_type, data);
            ////printf("%s\r\n", data);
            // printf("Server %d\r\n", setting.server);
            // printf("Type %d\r\n", setting.type);
            // break;
            ...
            case CMD_QT_TO_CPU_SET_LAMP_RS485_PORT_TO_GROUP:
                ...
                if(add_lamp_port_group.group == 1)
                {
                    if (add_lamp_port_group.port == 1)
                    {
                        ...
                        send_struct(CMD_CPU_TO_MASTER_SETTINGS_LAMP_GR_PORT1, (uint8_t *)&lamp_rf_group, sizeof(change_group_lamp_t));
                    }
                    else if(add_lamp_port_group.port == 2)
                    {
                        ...
                        send_struct(CMD_CPU_TO_MASTER_SETTINGS_LAMP_GR_PORT2, (uint8_t *)&lamp_rf_group, sizeof(change_group_lamp_t));
                    }
                ...
                /** delete lamp to group */
                else if (add_lamp_port_group.group == 0 && add_lamp_port_group.port == 1)
                {
                    lamp_rf_group.id = add_lamp_port_group.id;
                    lamp_rf_group.port = add_lamp_port_group.port;
                    lamp_rf_group.group = 200;
                    lamp_rf_group.dim = add.dim;
                    printf("gia tri queue rear lamp rs483: %d\r\n", queue_lamps_port1.rear);
                    for (int i = 0; i <= queue_lamps_port1.rear; i++)
                    {
                        if (queue_lamps_port1.inp_arr[i] == add_lamp_port_group.id)
                        {
                            queue_lamps_port1.inp_arr[i] = 0;
                            descrease_queue_lamp_port(add_lamp_port_group.port);
                            show_queue_lamp_port(add_lamp_port_group.port);
                            break;
                        }
                    }
                    send_struct(CMD_CPU_TO_MASTER_DIM_ID, (uint8_t *)&lamp_rf_group, sizeof(change_group_lamp_t));
                    Write_struct_queue_lamps_port1_toFile(queue_lamp_port1_file, "/home/pi/dim_group_lamp_port1.txt", queue_lamps_port1);
                    sort_up(queue_lamps_port1.inp_arr, queue_lamps_port1.rear);
                    // send_data_cpu_to_qt(queue_lamps_port1.inp_arr, queue_lamps_port1.rear, TEMPLATE_NUMBER_GROUP_PORT1, CMD_QT_TO_CPU_SET_LAMP_RS485_PORT_TO_GROUP);

                }
                else if (add_lamp_port_group.group == 0 && add_lamp_port_group.port == 2)
                {
                    lamp_rf_group.id = add_lamp_port_group.id;
                    lamp_rf_group.port = add_lamp_port_group.port;
                    lamp_rf_group.group = 200;
                    ...
                    for (int i = 0; i <= queue_lamps_port2.rear; i++)
                    {
                        if (queue_lamps_port2.inp_arr[i] == add_lamp_port_group.id)
                        {
                            queue_lamps_port2.inp_arr[i] = 0;
                            descrease_queue_lamp_port(add_lamp_port_group.port);
                            show_queue_lamp_port(add_lamp_port_group.port);
                            break;
                        }
                    ...
                    send_struct(CMD_CPU_TO_MASTER_DIM_ID, (uint8_t *)&lamp_rf_group, sizeof(change_group_lamp_t));
                    Write_struct_queue_lamps_port2_toFile(queue_lamp_port2_file, "/home/pi/dim_group_lamp_port2.txt", queue_lamps_port2);
                    sort_up(queue_lamps_port2.inp_arr, queue_lamps_port2.rear);
                    // send_data_cpu_to_qt(queue_lamps_port2.inp_arr, queue_lamps_port2.rear, TEMPLATE_NUMBER_GROUP_PORT2, CMD_QT_TO_CPU_SET_LAMP_RS485_PORT2_TO_GROUP);
...
int main()
{
    ...
    flag_checkreboot = setting.server;

    // printf("Preperal data time active send fist qt\r\n");
    // printf("hh_start is: %d\r\n", time_active.hh_start);
    // printf("nmm_start is: %d\r\n", time_active.mm_start);
    // printf("nhh_end is: %d\r\n", time_active.hh_end);
    // printf("nmm_end is: %d\r\n", time_active.mm_end);
